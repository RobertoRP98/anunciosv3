                ...
                <TableBody>
                    <TableRow v-for="state in states" :key="state.id">
                        ...
                        <TableCell class="space-x-2">
                            <Link :href="`/estados/${state.id}`" :class="buttonVariants({ variant: 'secondary' })
                                ">Show</Link>

                            <Link :href="`/estados/${state.id}/edit`" :class="buttonVariants({ variant: 'default' })
                                ">Edit</Link>

                            <Button variant="destructive" @click="deleteState(state.id)" class="ml-2">Delete</Button>
                        </TableCell>
                    </TableRow>
                </TableBody>
            </Table>
